...
use crate::sales::components::sales_order_filter::SalesOrderFilter;
use crate::sales::contexts::sales_order_filter_context::{use_sales_order_filter, PaymentStatusFilter};

#[function_component(SalesOrdersList)]
pub fn sales_orders_list() -> Html {
    let i18n = use_locale();
    let user_ctx = use_user_context();
    let navigator = use_navigator().unwrap();

    // Consume the filter context provided by the parent
    ...
    // Fetch orders dynamically based on context state
    let fetch_orders = {
        ...
        Callback::from(move |_: ()| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                // Build query params from filter_ctx state
                let status_str = match filter_ctx.status {
                    PaymentStatusFilter::Draft => "Draft",
                    PaymentStatusFilter::Outstanding => "Open",
                    PaymentStatusFilter::Paid => "Completed",
                    PaymentStatusFilter::All => "All",
                };

                let mut url = format!("/api/sales-orders?status={}", status_str);

                if let Some(partner_id) = filter_ctx.partner_id {
                    url.push_str(&format!("&partner_id={}", partner_id));
                }
                if let Some(min) = filter_ctx.min_amount {
                    url.push_str(&format!("&min_amount={}", min));
                }
    ...
    // Refetch whenever the filter context changes
    {
        let fetch_orders = fetch_orders.clone();
        use_effect_with(filter_ctx.clone(), move |_| {
            fetch_orders.emit(());
            || ()
        });
    }
    ...
    html! {
        <>
            ...
            <div class="table-actions">
                <button class="button-primary" onclick={on_new_order}>
                    { i18n.t("sales-orders-list-new-button") }
                </button>
            </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="table__value-col">{ i18n.t("common-due") }</th>
