...
pub struct AddUomModalProps {
    pub on_close: Callback<()>,
    pub on_submit: Callback<()>,
}

#[function_component(AddUomModal)]
pub fn add_uom_modal(props: &AddUomModalProps) -> Html {
    let user_ctx = use_user_context();
    let i18n = use_locale();
    let navigator = use_navigator().unwrap();
    let request = use_state(UnitOfMeasure::default);
    ...
    let on_form_submit = {
        ...
        Callback::from(move |e: SubmitEvent| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let resp = Api::post("/api/sales/uoms", &*request, user_ctx, navigator).await;
                if resp.is_ok() {
                    on_submit.emit(());
                }
            });
