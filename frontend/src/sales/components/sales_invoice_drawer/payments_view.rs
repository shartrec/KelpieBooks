...
pub fn payments_view(props: &PaymentsViewProps) -> Html {
    ...
    let request = use_state(|| {
        ...
        CreateCustomerPaymentRequest {
            ...
            reference: None,
            allocations: vec![CustomerPaymentAllocation {
                id: Uuid::new_v4(),
                organization_id: user.organization_id,
                sales_invoice_id: props.invoice.id,
                customer_payment_id: Uuid::nil(),
                allocated_amount: props.invoice.amount_due,
                created_at: Utc::now(),
            }],
        }
    });
        ...
        Callback::from(move |_: ()| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let fetched_payments = Api::get(
                    &format!("/api/sales-invoices/{}/payments", invoice_id),
                    user_ctx,
                    navigator,
                )
                .await;
                match fetched_payments {
                    Ok(response) if response.ok() => {
                        match response.json::<Vec<CustomerPayment>>().await {
    ...
    let on_input = |field_updater: fn(&mut CreateCustomerPaymentRequest, String)| {
    ...
    let on_date_change = |field_updater: fn(&mut CreateCustomerPaymentRequest, NaiveDate)| {
        let state = request.clone();
    ...
    let on_select_change = |field_updater: fn(&mut CreateCustomerPaymentRequest, String)| {
        let state = request.clone();
        Callback::from(move |e: Event| {
            let mut info = (*state).clone();
