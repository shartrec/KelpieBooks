...
use crate::{
    ...
    sales::components::sales_invoice_drawer::item_edit_card::ItemEditCard,
};
...
pub fn items_view(props: &ItemsViewProps) -> Html {
    ...
    let items = use_state(|| props.invoice.lines.clone());
    let accounts = use_state(Vec::new);
    let error = use_state(|| None::<String>);
    let invoice_id = props.invoice.id;
    let item_to_edit = use_state(|| None::<SalesInvoiceItem>);
    let item_to_delete = use_state(|| None::<SalesInvoiceItem>);
                ...
                match fetched_accounts {
                    Ok(response) if response.ok() => match response.json::<Vec<Account>>().await {
                        ...
                        Err(e) => error.set(Some(i18n.t_args(
                            "new-sales-invoice-error-parse-accounts",
                            &fluent_args!["error" => e.to_string()],
                        ))),
                    },
                    Ok(response) => error.set(Some(i18n.t_args(
                        "new-sales-invoice-error-fetch-accounts",
    ...
    let add_item = {
        let item_to_edit = item_to_edit.clone();
        Callback::from(move |_| {
            item_to_edit.set(Some(SalesInvoiceItem {
                id: Uuid::new_v4(),
                invoice_id,
    ...
    let on_submit = {
        ...
        Callback::from(move |e: SubmitEvent| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let resp = Api::put(
                    &format!("/api/sales-invoices/{}/items", invoice_id),
                    &*items,
                    user_ctx,
                    navigator,
                )
            ...
            if let Some(item) = &*item_to_edit {
                ...
                <form onsubmit={on_submit}>
                    { for (*items).iter().map(|item| {
                        ...
                        let gross_amount = item.net_amount + item.tax_amount;
                            ...
                            <div class="card-item-compact">
                                ...
                                <div class="card-item-compact__meta">
                                    <span class="card-item-compact__account-badge">
                                        { format!("{} - {}", item.quantity, item.name.clone()) }
                                    </span>
                                ...
                                <div class="card-item-compact__body">
                                    ...
                                    <div class="card-item-compact__financials">
                                        <p class="card-item-compact__total">
                                            { i18n.format_currency(gross_amount) }
                                        </p>
