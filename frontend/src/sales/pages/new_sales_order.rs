...
#[function_component(NewSalesOrderPage)]
pub fn new_sales_order_page() -> Html {
                    ...
                    Ok(response) if response.ok() => {
                        match response.json::<Vec<Warehouse>>().await {
                            ...
                            Err(e) => error.set(Some(i18n.t_args(
                                "new-sales-order-error-parse-warehouses",
                                &fluent_args!["error" => e.to_string()],
                            ))),
                        }
                    }
                    Ok(response) => error.set(Some(i18n.t_args(
                        "new-sales-order-error-fetch-warehouses",
    ...
    let on_input = |field_updater: fn(&mut CreateSalesOrderRequest, String)| {
        let state = request.clone();
    ...
    let on_date_change = |field_updater: fn(&mut CreateSalesOrderRequest, NaiveDate)| {
        let state = request.clone();
    ...
    let on_item_change = {
        ...
        Callback::from(move |item: SalesOrderItem| {
    ...
    html! {
        <Layout>
            <h1>{ i18n.t("new-sales-order-title") }</h1>
            ...
            if request.partner_id != PartnerId::default() {
                html! {
                    <>
                    <div class="data-form">
                        <label>{i18n.t("new-sales-order-date-label")}</label>
                        <input type="date" value={request.order_date.format("%Y-%m-%d").to_string()} onchange={on_date_change(|r, v| r.order_date = v)} required=true />
                        ...
                        <select onchange={on_warehouse_change} required=true>
                            ...
                            { for (*warehouses).iter().map(|wh| html! {
                                <option
                                    value={wh.id.to_string()}
                                    selected={request.warehouse_id == wh.id}
                                >
                                    { format!("{} — {}", wh.code, wh.name) }
                                </option>
                            }) }
                        </select>
                    </div>
                    ...
                    <div class="sale-footer">
                        ...
                        <button type="submit" class="button-primary">{ i18n.t("new-sales-order-save-button") }</button>
                    </div>
                </>
                }
                } else {
