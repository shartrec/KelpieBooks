...
use crate::services::account_service::{get_system_accounts, get_account_with_balance};

#[get("/financial-health")]
async fn get_financial_health(mut db: Connection<DbKelpie>, user: AuthenticatedUser) -> Result<Json<FinancialHealth>, rocket::http::Status> {
    ...
    let profit_loss_accounts = get_profit_loss(&mut db, org_id, year_start, today).await.unwrap_or_default();

    let revenue_total = profit_loss_accounts.iter()
        .filter(|a| a.category == AccountCategory::Revenue)
        .map(|a| a.balance)
        .sum::<i64>();

    let expense_total = profit_loss_accounts.iter()
...
async fn get_recent_transactions(mut db: Connection<DbKelpie>, user: AuthenticatedUser) -> Result<Json<Vec<RecentTransaction>>, rocket::http::Status> {
    let org_id = user.organization_id;

    let transactions = get_recent_transactions_from_db(&mut db, org_id, 5).await.unwrap_or_default();
    let mut recent_transactions = Vec::new();

    for tx in transactions {
        let journal_entries = get_all_by_transaction(&mut db, tx.id).await.unwrap_or_default();
...
async fn get_expense_breakdown(mut db: Connection<DbKelpie>, user: AuthenticatedUser) -> Result<Json<Vec<ExpenseBreakdown>>, rocket::http::Status> {
    let org_id = user.organization_id;
    let today = Local::now().date_naive();
...
pub fn routes() -> Vec<Route> {
    routes![get_financial_health, get_recent_transactions, get_expense_breakdown]
}
